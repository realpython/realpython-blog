...
> Remember that you can force a name for the migration with `./manage.py makemigrations --name <some name>`
...
Even though you dont' have to worry about the SQL that migrations generate, if your curioius Django's got you covered just run:

```sh
$ ./manage.py sqlmigration <app-name> <migration-name>
```

Doing that will list out SQL that will be generated by the specified migration, based upon the database in your settings.py file.  Run this a couple of times and you will really start to appreciate the power of migrations! :)
